...
select name as BandName from bands;
...
bands join albums on bands.id = albums.band_id
group by albums.band_id;


select bands.name from 
bands left join albums on bands.id = albums.band_id
where bands.id not in
	(select distinct albums.band_id from albums
);
...
update albums as a, (select id from albums where release_year = 2) as b
set a.release_year = 2018
where a.id = b.id;

select * from bands;

insert into bands(id,name) values(10, 'TestBand');

insert into albums(id, name, release_year, band_id) values (20, 'Eshwar', 2022, 10);
...
select songs.album_id, count(songs.id) from songs
group by songs.album_id;


select bands.name, count(songs.id)
